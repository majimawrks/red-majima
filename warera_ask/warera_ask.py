...
class WareraAsk(commands.Cog):
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "DATA NOT AVAILABLE via these endpoints (do NOT guess — say it's unavailable):\n"
            "- Player-owned companies / business listings per country. The `country.orgs` "
            "  field is NOT player companies — it appears to be 1 national-level org per "
            "  country, not a directory of businesses. There is no company.* endpoint.\n"
            "- Per-region company/factory counts. Regions don't expose company data.\n"
            "- `user.skills.companies` is a player SKILL stat (level/value), NOT a count "
            "  of companies the user owns. Do not use it as a company count.\n\n"
            "WORKFLOW HINTS:\n"
            ...
            "- Most answerable questions need only 2-3 endpoint calls.\n\n"
            "RULES:\n"
            ...
            "- Before giving up, check the terminology glossary and search.searchAnything. "
            "  But if a question asks for data listed under 'DATA NOT AVAILABLE' above, "
            "  say so honestly — do NOT substitute a different field that has a similar name.\n"
            "- NEVER guess. If `country.orgs` has 1 entry, that is 1 org — it is NOT proof "
            "  there is 1 company. Counting the wrong field is worse than admitting you don't know.\n"
